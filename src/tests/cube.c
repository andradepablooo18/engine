...
void zoom_cube_with_mouse_wheel(Input* input, f32 dt, f32 zoom_speed);
void rotate_cube_when_pressed(Input* input, f32 dt, f32 sensivity);
...
void update(Input* input, f32 dt) {
    ...
    zoom_cube_with_mouse_wheel(input, dt, ZOOM_SPEED);
    rotate_cube_when_pressed(input, dt, SENSIVITY);
...
void zoom_cube_with_mouse_wheel(Input* input, f32 dt, f32 zoom_speed) {
    ...
    Object3D_scale(cube, (Vector3){dt * wheel_y * zoom_speed,
                                   dt * wheel_y * zoom_speed,
                                   dt * wheel_y * zoom_speed});
}
...
void rotate_cube_when_pressed(Input* input, f32 dt, f32 sensivity) {
    if (Input_is_mouse_button_down(input, MOUSE_BUTTON_LEFT)) {
        ...
        Object3D_rotate(cube, (Vector3){dt * delta.y * sensivity,
                                        dt * delta.x * sensivity, 0.0f});
    }
}
